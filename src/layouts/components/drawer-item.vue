<template>
  <div class="drawer-item" v-if="!item.meta||!item.meta.hidden">
    <q-expansion-item v-if="isExpansion(item.children)" expand-separator
      :default-opened="isOpen()" :dense="dense"
      ...
      <drawer-item v-for="(e,i) in item.children" :key="i" :dense="dense" :item="e"
        :active="onActive(item.name)"
        :base-path="`${basePath?`${basePath}/`:''}${item.path}`" is-icon />
    </q-expansion-item>
    <q-item v-else clickable v-ripple @click="onRouterLink(item.path)"
      :active="onActive(item.name)" :dense="dense">
      <q-item-section avatar>
