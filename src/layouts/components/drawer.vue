  ...
  <q-list class="rounded-borders">
    <template v-for="(item,index) in items">
      <q-expansion-item v-if="isExpansion(item.children)" :key="index" expand-separator
        :header-class="onActive(item.name)?'q-item--active':''"
        :default-opened="onActive(item.name)" :dense="dense">
        ...
        <template v-for="(c,ii) in item.children">
          <q-expansion-item v-if="isExpansion(c.children)" :key="ii" expand-separator
            switch-toggle-side dense-toggle :header-inset-level=".1"
            :header-class="onActive(c.name)?'q-item--active':''"
            ...
            <template v-for="(cc,iii) in c.children">
              <drawer-item :key="iii" :dense="dense" :item="cc"
                :basePath="`${item.path}/${c.path}`" :active="onActive(cc.name)"
                :inset-level=".1" />
            </template>
          </q-expansion-item>
          <drawer-item v-else :key="ii" :dense="dense" :item="c" :base-path="item.path"
            :active="onActive(c.name)" :inset-level=".5" />
      ...
      <drawer-item v-else :key="index" :item="item" :active="onActive(item.name)"
        :dense="dense" />
  ...
  methods: {
    ...
    isExpansion(children) {
      if (children && children.length) {
        const x = children.filter(x => !x.hidden)
