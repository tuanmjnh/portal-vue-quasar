...
export default {
  ...
  methods: {
    onFileChange(e) {
      ...
      // this.file.name.
      $this.$emit('on-start', $this.file)
      const type = getExtension($this.file.name, false)
      if (type === 'xls' || type === 'xlsx' || type === 'csv' || type === 'tsv') {
        ...
        reader.onload = (e) => {
          ...
          const sheet = XLSX.utils.sheet_to_json(worksheet, this.options)
          $this.$emit('on-finish', sheet)
          $this.loading = false
        }
