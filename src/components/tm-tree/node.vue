<template>
  ...
  <div class="tm-node" v-if="node.show">
    <div :class="['tm-node-content',node.children&&node.children.length?'tm-node-expand':'']">
      <div :class="selected===node[label]?'selected':''">
        ...
        <div class="node-item" @click="onClickNode" @dblclick="onMakeFolder">
          ...
          <div v-if="!slotContentAfter" class="node-label">{{ node[label] }}</div>
        </div>
      </div>
    </div>
    ...
    <draggable v-if="draggable" :list="node.children" :group="{ name: 'people' }" tag="div"
      ...
      <tm-tree-node v-show="node.expanded" v-for="(child, index) in node.children" :key="index" :node="child"
        node-key="_id" :label="label" :is-children="true" :selected.sync="selected" :ticked.sync="ticked"
...
export default {
  ...
  created() {
    // selected
    if (this.selected === this.node[this.label]) this.node.selected = true
    // expanded
    if (this.expandedAll) this.node.expanded = true
    else {
      if (this.expanded && this.expanded.length && this.expanded.includes(this.node[this.label])) this.node.expanded = true
      // if (this.expanded.includes(this.node[this.label])) this.isOpen = true
    }
    // ticked
    if (this.ticked && this.ticked.length) {
      if (this.ticked.includes(this.node[this.label])) {
  ...
  watch: {
    // ticked(val) {
    //   if (this.ticked.includes(this.node[this.label])) this.node.ticked = true
    //   else this.node.ticked = false
    // }
    // parent: {
    //   handler(val) {
  ...
  methods: {
    ...
    onClickNode() {
      ...
      // if (this.selected === this.node[this.label]) this.$emit('update:selected', null)
      // else this.$emit('update:selected', this.node[this.label])
