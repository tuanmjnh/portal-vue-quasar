<template>
  ...
  <div class="tm-node" v-if="node.show">
    <div
      :class="['tm-node-content',node.children&&node.children.length?'tm-node-expand':'']">
      <div :class="selected===node[nodeKey]?'selected':''">
        <div class="node-expand">
          <i v-if="node.children&&node.children.length"
            :class="['material-icons tree-arrow',node.expanded?'open':'']"
            @click="onToggle">play_arrow</i>
        </div>
        <!-- <i v-else class="node-prefix"></i> -->
        <q-checkbox v-model="node.ticked" v-if="ticked!==undefined"
    ...
    <draggable v-if="draggable" :list="node.children" :group="{ name: 'people' }"
      tag="div" class="tm-nodes tm-node-child" transition-show="flip-up"
      transition-hide="flip-down" @change="$emit('on-drag-changed', $event)">
      ...
      <q-btn v-if="addButton" flat round size="sm" color="primary" icon="add"
        class="node-add" />
      <!-- </div> -->
    </draggable>
...
export default {
  ...
  created() {
    // selected
    if (this.selected === this.node[this.nodeKey]) this.node.selected = true
    // expanded
